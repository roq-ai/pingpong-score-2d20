...
import { deleteGameById } from 'apiSdk/games';
...
function OrganizationViewPage() {
  ...
  const gameHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('game', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/games/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.game?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/games/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            <Td>{record.score}</Td>
                            <Td>
